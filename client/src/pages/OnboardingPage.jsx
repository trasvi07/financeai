...
function generateSmartBudget(profile) {
  const {
    ...
    lifestyle = 'moderate', housingType = 'rented',
    dependents = 0, goals = []
  } = profile
  ...
  const ratios = { ...lifestyleRatios[lifestyle] }

  if (housingType === 'rented')    ratios['Utilities']   += 0.05
  if (dependents > 0)              ratios['Food & Dining'] += dependents * 0.02
  if (goals.includes('Invest More') || goals.includes('Retire Early')) ratios['Investment'] += 0.05
  if (goals.includes('Build Emergency Fund')) ratios['Investment'] += 0.03
  if (goals.includes('Pay Off Debt'))         ratios['Investment'] += 0.04
...
export default function OnboardingPage() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Always mark complete and redirect — even if API fails
          ...
          {step === 0 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-gray-400">This is the foundation of your budget model</p>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1.5">
                  Other Income
                  <span className="text-gray-500 font-normal ml-1 text-xs">(freelance, rent, etc.) — optional</span>
                </label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-medium">
                    {currSymbol}
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-gray-400">Helps AI understand your fixed monthly costs</p>
              </div>
              ...
              {formData.housingType === 'rented' && (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                  <p className="text-sm text-amber-400">
                    ⚠️ Rent costs will be treated as a mandatory fixed expense in your budget
                  </p>
                </div>
              )}
              ...
              <div className="bg-gray-800/60 rounded-xl p-4 space-y-2">
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wide mb-2">Profile So Far</p>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-indigo-400">
                    {formData.dependents === '0' ? 'None' : formData.dependents}
                  </span>
                </div>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-gray-400">AI uses this to set realistic category limits</p>
              </div>
